...
Every node in a Kubernetes cluster has a running **Kubelet Service** which is
responsible for managing the containers scheduled to run on each node. These
options let you customize some of the cluster-wide Kubelet settings.

Here's what the Kubelet options look like, with the default values or
