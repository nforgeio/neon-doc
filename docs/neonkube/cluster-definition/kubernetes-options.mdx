...
description: NeonKUBE Kubernetes options
last_update:
...
Here's what the Kubernetes options look like, with the default values or
...
:::note 
**allowPodsOnControlPlane** defaults to **false** for cluster that have
worker nodes or **true** for clusters with only control-plane nodes (such as the
desktop cluster). 
:::
...
<table>
    ...
    <tr>
        ...
        <td>
            `map:` Used to specify hard eviction thresholds that Kubernetes will use to evict pods with our
    ...
    <tr>
        ...
        <td>
            `integer:` Specifies the seconds that Kubernetes will delay node shutdown for critical pods.
    ...
    <tr>
        ...
        <td>
            `integer:` Specifies seconds Kubernetes will delay node shutdown while gracefully terminating pods
    ...
    <tr>
        <td><b>terminatedPodGcThreshold</b></td>
        <td>
