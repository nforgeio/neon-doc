...
We'll use the **[AZURE_CLIENT_ID]**, **[AZURE_CLIENT_SECRET]**, and **[AZURE_TENANT_ID]** from
the output plus the **AZURE_SUBSCRIPTION_ID** you already know.

Next, you'll need to accept the NEONKUBE offer terms and EULA, via the AZURE-CLI:

```plain
REM View the offer terms:
az vm image terms show --urn neonforge:neonkube:0-11-0-alpha-0:0.11.0

REM Accept the terms
az vm image terms accept --urn neonforge:neonkube:0-11-0-alpha-0:0.11.0
```

:::note
Azure doesn't directly support semantic versioning of their Virtual Machine Images,
so we have to do some version contortions to get this to work.  We'll probably add
a NEONCLI command that figures out the version and then launches AZURE-CLI with the
correct paramweters.

The **0-11-0-alpha-0:0.11.0** version part of the URN above is formatted with the
first part being the full semantic version of the release *0.11.0-alpha.0* with
all dots replaced by dashes like *0-11-0-alpha-0* and the second part being 
the **major**, **minor**, and **patch** version of the release only.
:::
...
- Configuring the hosting infrastructure, which involves configuring the 
  **Resource Group**, **Virtual Networl**, starting the **VMs**, configuring 
